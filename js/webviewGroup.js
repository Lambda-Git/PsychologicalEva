var webviewGroup = function(id, options) {
	...
	this.options.index = options.task_list || 0;
...
proto._init = function() {
	...
	this._initWebviewContexts(this.options.task_list);
};
...
proto._initWebviewContexts = function() {
	for(var len = this.items.length, i = len - 1; i >= 0; i--) {
		...
		var isFirst = i === 0;
		var isLast = i === (len - 1);
		var isCurrent = this.options.task_list === i;
		...
		if(i > this.options.task_list) {
			styles.left = '100%';
		} else if(i < this.options.task_list) {
			styles.left = '-100%';
		} else {
			styles.left = '0';
		}
